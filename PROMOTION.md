...
- v1.3.0 rebuilds the verification report end to end (aligned with official-doc-writer 3.7.0 + 3.6.2, renderer 1627 → 2524 lines): dual views (verification report + standalone material library with per-retrieval-group tabs, search, real keyword chips, cited/uncited filters), continuous document flow with sentence-anchored citation pills that expand source cards in place (multi-segment excerpts, breadcrumb title chains, view-full-text / archived snapshot), per-chapter citation badges, copy-clean-text, print modes and reading progress. Verification verdicts graded: normal conclusion is "核验完成" (green) — missing source links no longer drag down the verdict (yellow note + material flag), only missing excerpts fail. Preprocessing built in: policy document-number matching, link liveness checking (404/410 + soft-404 sniffing, ~10s, `--skip-link-check`), archived-snapshot fallback (`--no-snapshot`), knowledge-base re-view injection. `trusted_search.py` now defaults `simplified=false` (full result set carries `screenShotPath` snapshots the fallback needs; `--simplified` opts into the lossy compact set).
